...
@Component({
  ...
  templateUrl: './first-cols.component.html',
  styleUrls: ['./first-cols.component.scss']
})
export class FirstColsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.store.pipe(select(getCurrentCompetition)).subscribe((res) => {
      if (res) {
        ...
        console.log(this.viewsForDesktop);
